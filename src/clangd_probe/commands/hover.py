...
from .common import backend_factory, prepare_semantic_context, semantic_symbol_search


def run(args, context):
    preflight = prepare_semantic_context("hover", args, context)
    ...
    try:
        with backend_factory(context, initial_file=initial_file) as backend:
            ...
            if resolved.status != "ok":
                return CommandResult(command="hover", status=resolved.status, results=resolved.results, diagnostics=resolved.diagnostics)
            target = resolved.results[0]
            result = backend.hover(target["path"], target["line"], target["column"])
    ...
    if result is None:
        return CommandResult(command="hover", status="no_results")

    return CommandResult(command="hover", status="ok", results=[result])
